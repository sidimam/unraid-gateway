...
RUN chmod +x /usr/local/bin/entrypoint.sh /usr/local/bin/gw
# The entrypoint starts as root only to open Unraid's notification spool for user 99, then drops to
# Unraid's default "nobody:users" (99:100) so files created through the gateway get the same
# ownership as files created via SMB.
ENV LISTEN_ADDR=:8484 DATA_ROOT=/data UNRAID_NOTIFY_DIR=/unraid-notifications
EXPOSE 8484
VOLUME ["/data"]
