...
Mount each share at `/data/<name>`. `<name>` is what clients see; it may differ from the share name. Use `:ro` for read-only. Nothing outside `/data` is ever touched; the container needs no other mounts.
